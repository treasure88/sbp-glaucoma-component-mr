...
echo "Running SBP pre-harmonization QC..."
...
vcf = "../../07_exposure_gwas_raw/SBP__ieu-b-38.vcf.gz"

exposure_id = "SBP"
file_path = "../../07_exposure_gwas_raw/SBP__ieu-b-38.vcf.gz"
...
# LP is -log10(P); original pval is not stored directly in VCF.
# For QC summary, report minimum p as 10^(-max_LP).
if max_lp is None:
    pval_min = "NA"
else:
...
print("\t".join([
    ...
    sample_size,
    population,
    "OpenGWAS ieu-b-38; pval derived from LP; ALT is effect allele",
    "REGISTERED"
]))
print("")
print("===== qc_row =====")
...
# Remove existing SBP rows if rerunning
awk -F '\t' 'NR==1 || $1!="SBP"' "$REG" > "$REG.tmp"
mv "$REG.tmp" "$REG"

awk -F '\t' 'NR==1 || $1!="SBP"' "$QC" > "$QC.tmp"
mv "$QC.tmp" "$QC"
