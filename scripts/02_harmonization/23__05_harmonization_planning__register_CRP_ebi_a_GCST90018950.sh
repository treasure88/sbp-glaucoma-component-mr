...
echo "Running CRP pre-harmonization QC..."

python - << 'PY' > "$LOG"
import gzip
import re

vcf = "../../07_exposure_gwas_raw/CRP__ebi-a-GCST90018950.vcf.gz"

exposure_id = "CRP"
file_path = "../../07_exposure_gwas_raw/CRP__ebi-a-GCST90018950.vcf.gz"
...
qc_note = (
    ...
    "no per-variant SS in FORMAT, use study-level N=353466 from TotalControls; "
    "pval will be computed as 10^(-LP) during exposure standardization"
)

print("===== CRP pre-QC summary =====")
...
print("\t".join([
    ...
    sample_size,
    population,
    "OpenGWAS ebi-a-GCST90018950; continuous CRP; pval derived from LP; ALT is effect allele; no per-variant SS, use study-level N",
    "REGISTERED"
]))
...
awk -F '\t' 'NR==1 || $1!="CRP"' "$REG" > "$REG.tmp"
mv "$REG.tmp" "$REG"

awk -F '\t' 'NR==1 || $1!="CRP"' "$QC" > "$QC.tmp"
mv "$QC.tmp" "$QC"
