...
all_file <- file.path(log_dir, "MIGRAINE__ebi-a-GCST90038646_all_opengwas_urls.txt")
vcf_file <- file.path(log_dir, "MIGRAINE__ebi-a-GCST90038646_vcf_url.txt")
tbi_file <- file.path(log_dir, "MIGRAINE__ebi-a-GCST90038646_tbi_url.txt")

writeLines(char_vals, all_file)

vcf_url <- char_vals[grepl("ebi-a-GCST90038646\\.vcf\\.gz($|\\?)", char_vals)]
tbi_url <- char_vals[grepl("ebi-a-GCST90038646\\.vcf\\.gz\\.tbi($|\\?)", char_vals)]

if (length(vcf_url) < 1) {
  stop("Could not find ebi-a-GCST90038646.vcf.gz URL")
}
