...
all_file <- file.path(log_dir, "INSOMNIA__ebi-a-GCST90018869_all_opengwas_urls.txt")
vcf_file <- file.path(log_dir, "INSOMNIA__ebi-a-GCST90018869_vcf_url.txt")
tbi_file <- file.path(log_dir, "INSOMNIA__ebi-a-GCST90018869_tbi_url.txt")

writeLines(char_vals, all_file)

vcf_url <- char_vals[grepl("ebi-a-GCST90018869\\.vcf\\.gz($|\\?)", char_vals)]
tbi_url <- char_vals[grepl("ebi-a-GCST90018869\\.vcf\\.gz\\.tbi($|\\?)", char_vals)]

if (length(vcf_url) < 1) {
  stop("Could not find ebi-a-GCST90018869.vcf.gz URL")
}
