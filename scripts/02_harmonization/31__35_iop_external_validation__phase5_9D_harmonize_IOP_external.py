...
BASE = "../../36_iop_external_validation_inputs"
...
IOP_STD = "../../27_external_outcome_standardized/IOP__GCST009413.standardized.tsv.gz"
...
STATUS_OUT = os.path.join(BASE, "phase5_9D_status.tsv")
QC_OUT = os.path.join(BASE, "phase5_9D_IOP_harmonization_qc_summary.tsv")
RUNTIME_OUT = os.path.join(BASE, "phase5_9D_runtime_log.tsv")
...
def read_iop_for_snps(target_snps):
    matched = {}
    n_scan = 0

    with gzip.open(IOP_STD, "rt", encoding="utf-8", errors="replace", newline="") as f:
        reader = csv.DictReader(f, delimiter="\t")
        for r in reader:
            n_scan += 1
            snp = r.get("SNP", "")
...
def evaluate_match(inst, out):
    ...
    # Strand complement rescue for A/C/G/T single-base alleles.
...
def main():
    ...
    print(f"Scanning IOP standardized file for {len(all_snps)} target SNPs", flush=True)
    iop_matches, n_scan = read_iop_for_snps(all_snps)
    ...
    mr_fields = [
        ...
        "harmonization_action", "match_mode", "match_key",
        "outcome_source_dataset", "F_stat", "instrument_note",
        "outcome_scale", "outcome_maf", "outcome_n", "palindromic_possible"
    ]

    missing_fields = [
        "exposure_id", "outcome_id", "SNP", "reason", "effect_allele", "other_allele"
    ]
    ...
    for spec in INSTRUMENT_SOURCES:
        ...
        for inst in instruments:
            snp = inst["SNP"]
            outcome_rows = iop_matches.get(snp, [])
            ...
            row = {
                ...
                "outcome_source_dataset": "IOP_GCST009413",
                "F_stat": inst["F_stat"],
        ...
        hfile = os.path.join(PAIRWISE_DIR, f"{exposure_id}__IOP.targeted_external_harmonized.tsv.gz")
        mfile = os.path.join(MR_INPUT_DIR, f"{exposure_id}__IOP.external_mr_input.tsv.gz")
        missfile = os.path.join(MISSING_DIR, f"{exposure_id}__IOP.missing_external_instruments.tsv")
        ...
        qc_rows.append({
            ...
            "status": status,
            "note": "IOP external validation targeted harmonization using GCST009413",
        })
    ...
    with open(STATUS_OUT, "w", encoding="utf-8", newline="") as f:
        ...
        f.write(f"Phase 5.9D targeted IOP harmonization\tPASSED\t{ready_count}/2 pairwise IOP MR inputs ready; total_rows={total_rows}\tSBP and ART_STIFFNESS instruments harmonized against IOP\n")
        f.write("external_outcome\tINFO\tIOP\tMeasured intraocular pressure external validation layer\n")
        ...
        f.write("eaf_caveat\tDOCUMENTED\tIOP_EAF_missing_MAF_retained\tOutcome MAF not interpreted as EAF\n")
        f.write(f"runtime\tINFO\t{elapsed_all:.3f}s\tPhase 5.9D runtime\n")

    print("===== Phase 5.9D completed =====")
