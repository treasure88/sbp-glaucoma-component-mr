...
VCF="$RAW/MIGRAINE__ebi-a-GCST90038646.vcf.gz"
REG="$PLAN/phase2_exposure_file_registration.tsv"
QC="$PLAN/phase2_exposure_file_qc_summary.tsv"
LOG="$LOGDIR/MIGRAINE__ebi-a-GCST90038646_pre_qc.txt"
...
echo "Running MIGRAINE pre-harmonization QC..."

python - << 'PY' > "$LOG"
import gzip
import re

vcf = "../../07_exposure_gwas_raw/MIGRAINE__ebi-a-GCST90038646.vcf.gz"

exposure_id = "MIGRAINE"
file_path = "../../07_exposure_gwas_raw/MIGRAINE__ebi-a-GCST90038646.vcf.gz"
...
qc_note = (
    ...
    "no per-variant SS in FORMAT, use study-level N=484598 from TotalCases+TotalControls; "
    "pval will be computed as 10^(-LP) during exposure standardization"
)

print("===== MIGRAINE pre-QC summary =====")
...
print("\t".join([
    ...
    sample_size,
    population,
    "OpenGWAS ebi-a-GCST90038646; case-control migraine; pval derived from LP; ALT is effect allele; no per-variant SS, use study-level N",
    "REGISTERED"
]))
...
awk -F '\t' 'NR==1 || $1!="MIGRAINE"' "$REG" > "$REG.tmp"
mv "$REG.tmp" "$REG"

awk -F '\t' 'NR==1 || $1!="MIGRAINE"' "$QC" > "$QC.tmp"
mv "$QC.tmp" "$QC"
