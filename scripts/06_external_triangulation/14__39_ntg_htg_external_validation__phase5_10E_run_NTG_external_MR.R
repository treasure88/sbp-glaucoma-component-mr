OUTDIR <- "../../41_ntg_external_mr_results"
dir.create(OUTDIR, recursive = TRUE, showWarnings = FALSE)

INPUTS <- list(
  SBP = "../../40_ntg_external_validation_inputs/mr_input/SBP__NTG.external_mr_input.tsv.gz",
  ART_STIFFNESS = "../../40_ntg_external_validation_inputs/mr_input/ART_STIFFNESS__NTG.external_mr_input.tsv.gz"
)

RESULTS_OUT <- file.path(OUTDIR, "phase5_10E_NTG_external_mr_results.tsv")
HET_OUT <- file.path(OUTDIR, "phase5_10E_NTG_external_heterogeneity.tsv")
EGGER_OUT <- file.path(OUTDIR, "phase5_10E_NTG_external_egger_intercept.tsv")
SINGLE_OUT <- file.path(OUTDIR, "phase5_10E_NTG_external_single_snp_ratios.tsv.gz")
LOO_OUT <- file.path(OUTDIR, "phase5_10E_NTG_external_leave_one_out.tsv.gz")
STATUS_OUT <- file.path(OUTDIR, "phase5_10E_status.tsv")
RUNTIME_OUT <- file.path(OUTDIR, "phase5_10E_runtime_log.tsv")
...
ivw_result <- function(dat, exposure_id, random = FALSE) {
  ...
  data.frame(
    ...
    note = ifelse(random, "Primary external NTG MR method", "External NTG MR; fixed effects"),
    stringsAsFactors = FALSE
  )
}
...
single_snp_rows <- function(dat, exposure_id) {
  beta <- dat$beta_outcome / dat$beta_exposure
  se <- abs(dat$se_outcome / dat$beta_exposure)
...
writeLines(paste(
  "Phase 5.10E NTG external MR",
  "PASSED",
  paste0("ivw_random_tests=", nrow(ivw_random), ";nominal=", nominal, ";fdr=", fdr),
  "SBP and ART_STIFFNESS tested against NTG",
  sep = "\t"
), con)
writeLines(paste("outcome", "INFO", "NTG", "External normal tension glaucoma validation outcome", sep = "\t"), con)
writeLines(paste("outcome_scale", "DOCUMENTED", "log_odds_or_MTAG_scale_TO_VERIFY", "BETA from MTAG_NTG_IGGC_STAGE2.tab; interpret cautiously", sep = "\t"), con)
writeLines(paste("SBP_NTG_direction", "INFO", ifelse(nrow(sbp) > 0, sbp$direction[1], "NA"), "Key route-A NTG validation direction", sep = "\t"), con)
writeLines(paste("ART_STIFFNESS_NTG_direction", "INFO", ifelse(nrow(art) > 0, art$direction[1], "NA"), "Exploratory vascular-stiffness validation direction", sep = "\t"), con)
writeLines(paste("HTG_status", "TO_DO", "full_summary_stats_not_locked", "HTG validation remains unresolved", sep = "\t"), con)
writeLines(paste("runtime", "INFO", paste0(round(elapsed_all, 3), "s"), "Phase 5.10E runtime", sep = "\t"), con)
close(con)

write.table(runtime, RUNTIME_OUT, sep = "\t", row.names = FALSE, quote = FALSE)

cat("===== Phase 5.10E completed =====\n")
